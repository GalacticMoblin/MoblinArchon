...
global struct Archon_BarTopoData {
    vector position = Vector( 0.0, 0.0, 0.0 )
    vector size = Vector( 0.0, 0.0, 0.0 )
    vector angles = Vector( 0.0, 0.0, 0.0 )
    int segments = 1
    array<var> imageRuis
    array<Archon_TopoData> topoData
    int direction
	float fill
...
void function ArchonPrecache()
{
	...
	#endif
	#if CLIENT
		//AddCallback_PlayerClassChanged( Archon_ToggleUI )
...
/*#if CLIENT
void function Archon_ToggleUI( entity player )
{
	if ( player.IsTitan() )
	{
		if(GetCurrentPlaylistVarInt("aegis_upgrades", 0) == 1)
			thread ArchonRUIThink( player )
	}
}

void function ArchonRUIThink( entity player )
{
	player.EndSignal( "SettingsChanged" )
	player.EndSignal( "OnDestroy" )
	player.EndSignal( "OnDeath" )

	Archon_BarTopoData bg = Archon_BasicImageBar_CreateRuiTopo( <0, 0, 0>, < -0.417, 0.47, 0.0 >, 0.106, 0.015, Archon_eDirection.right )
	Archon_BarTopoData charge = Archon_BasicImageBar_CreateRuiTopo( <0, 0, 0>, < -0.417, 0.47, 0.0 >, 0.1, 0.01, Archon_eDirection.right )
	...
	OnThreadEnd(
		function() : ( bg, charge, text )
		{
			Archon_BasicImageBar_Destroy( bg )
			Archon_BasicImageBar_Destroy( charge )
			RuiDestroy( text )
		}
	)

	Archon_BasicImageBar_UpdateSegmentCount( bg, 1, 0.108 )
	RuiSetFloat3( bg.imageRuis[0], "basicImageColor", < 0, 0, 0 > )
	RuiSetFloat( bg.imageRuis[0], "basicImageAlpha", 0.35 )

	Archon_BasicImageBar_UpdateSegmentCount( charge, 1, 0.1 )
	RuiSetFloat3( charge.imageRuis[0], "basicImageColor", < 0.4, 0.4, 0.4 > )
	RuiSetFloat( charge.imageRuis[0], "basicImageAlpha", 1.0 )

	float percent = player.GetPlayerNetFloat( "coreMeterModifier" )
	if ( percent == 1 )
		RuiSetFloat3( charge.imageRuis[0], "basicImageColor", < 0.984, 0.753, 0.431 > )

	float oldPercent = percent
	Archon_BasicImageBar_SetFillFrac( charge, percent )

	RuiSetString( text, "msgText", "" )
	RuiSetFloat3( text, "msgColor", <0.7 + percent * 0.3, 0.7, 0.7 - percent * 0.5> )

	while( true )
	{
		WaitFrame()
		percent = player.GetPlayerNetFloat( "coreMeterModifier" )

		Archon_BasicImageBar_SetFillFrac( charge, percent )
		if ( percent == 1 && oldPercent != 1 )
		{
			RuiSetFloat3( charge.imageRuis[0], "basicImageColor", < 0.984, 0.753, 0.431 > )
			RuiSetString( text, "msgText", "TERMINATOR: ACTIVE" )
			RuiSetFloat3( text, "msgColor", <0.7 + percent * 0.3, 0.7, 0.7 - percent * 0.5> )
		}
